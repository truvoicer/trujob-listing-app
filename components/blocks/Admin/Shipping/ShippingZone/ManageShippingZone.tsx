...
function ManageShippingZone({
    ...
    function getShippingZoneFormModalProps(index?: number) {
        return {
            formProps: {
                operation: operation,
                ...
                switch (mode) {
                    case 'selector':
                        DataManagerService.selectorModeHandler({
                            onChange,
                            data,
                            values: formHelpers?.values?.shippingZones,
                            index
                        });
                        break;
                    case 'edit':
    ...
    function renderActionColumn(item: ShippingZone, index: number, dataTableContextState: DataTableContextType) {
        return (
                ...
                <Link className="badge bg-success-light mr-2"
                    ...
                    onClick={e => {
                        ...
                        dataTableContextState.modal.show({
                            ...
                            ...getShippingZoneFormModalProps(index),
                        }, EDIT_SHIPPING_ZONE_MODAL_ID);
                    }}
                >
                ...
                <Link className="badge bg-danger-light mr-2"
                    ...
                    onClick={e => {
                        ...
                        dataTableContextState.modal.show({
                            ...
                            component: (
                                <p>Are you sure you want to delete this shipping zone ({item?.name} | {item?.label})?</p>
                            ),
                            onOk: async () => {
                                console.log('Delete shipping zone', { operation, item });
                                if (!operation) {
                                    console.warn('Operation is required');
                                    return;
                                }
                                if (Array.isArray(data) && data.length) {
                                    ...
                                    dataTableContext.modal.close(DELETE_SHIPPING_ZONE_MODAL_ID);
                                    return;
                                }
                                if (!item?.id) {
                                    notificationContext.show({
                                        ...
                                        title: 'Error',
                                        component: (
                                            <p>Shipping zone ID is required</p>
                                        ),
                                    }, 'shipping-zone-delete-error');
                                    return;
                                }
                                ...
                                if (!response) {
                                    notificationContext.show({
                                        ...
                                        title: 'Error',
                                        component: (
                                            <p>Failed to delete shipping zone</p>
                                        ),
                                    }, 'shipping-zone-delete-error');
                                    return;
                                }
                            ...
                            linkProps: {
                                ...
                                onClick: e => {
                                    ...
                                    appModalContext.show({
                                        title: 'Delete shipping zone',
                                        component: (
                                            <p>Are you sure you want to delete this shippingZone ({item?.title})?</p>
                                        ),
                                        onOk: async () => {
                                            if (!item?.id) {
                                                notificationContext.show({
                                            ...
                                            const response = await TruJobApiMiddleware.getInstance().resourceRequest({
                                                endpoint: `${truJobApiConfig.endpoints.shippingZone}/${item.id}/delete`,
                                                method: ApiMiddleware.METHOD.DELETE,
                                                protectedReq: true
                                            })
                                            if (!response) {
                                                notificationContext.show({
                                                    ...
                                                    title: 'Error',
                                                    component: (
                                                        <p>Failed to delete shippingZone</p>
                                                    ),
                                                }, 'shipping-zone-delete-error');
                                                return;
                                            }
    ...
    async function prepareSearch(searchParams: DatatableSearchParams = {}) {
        ...
        if (isNotEmpty(searchParams?.shippingZone)) {
            query['shippingZone'] = searchParams.shippingZone;
        }
        return query;
    }
    async function shippingZoneRequest({ dataTableContextState, setDataTableContextState, searchParams }: {
    ...
    function renderAddNew(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, { dataTableContextState, setDataTableContextState }: {
        ...
        modalState.show({
            ...
            ...getShippingZoneFormModalProps(),
        }, CREATE_SHIPPING_ZONE_MODAL_ID);
    }
        ...
        actions.push({
            ...
            onClick: ({
                ...
                dataTableContextState.confirmation.show({
                    title: 'Edit Menu',
                    message: 'Are you sure you want to delete selected shippingZones?',
                    onOk: async () => {
                        console.log('Yes')
                        if (!data?.length) {
                            notificationContext.show({
                                ...
                                title: 'Error',
                                component: (
                                    <p>No shippingZones selected</p>
                                ),
                            }, 'shipping-zone-bulk-delete-error');
                            return;
                        }
                        ...
                        if (!ids?.length) {
                            notificationContext.show({
                                ...
                                title: 'Error',
                                component: (
                                    <p>Shipping zone IDs are required</p>
                                ),
                            }, 'shipping-zone-bulk-delete-error');
                            return;
                        }
                        const response = await TruJobApiMiddleware.getInstance().resourceRequest({
                            endpoint: `${truJobApiConfig.endpoints.shippingZone}/bulk/delete`,
                            method: ApiMiddleware.METHOD.DELETE,
                            protectedReq: true,
                            data: {
                                ids: ids
                        ...
                        if (!response) {
                            notificationContext.show({
                                ...
                                title: 'Error',
                                component: (
                                    <p>Failed to delete shippingZones</p>
                                ),
                            }, 'shipping-zone-bulk-delete-error');
                            return;
                        }
                        ...
                        notificationContext.show({
                            ...
                            title: 'Success',
                            component: (
                                <p>Shipping zones deleted successfully</p>
                            ),
                        }, 'shipping-zone-bulk-delete-success');
                        dataTableContextState.refresh();
                    },
                    onCancel: () => {
                        console.log('Cancel delete');
                    },
                }, 'delete-bulk-shipping-zone-confirmation');
            }
        });
        return actions;
    }
