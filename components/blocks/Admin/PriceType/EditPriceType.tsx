...
function EditPriceType({
    ...
    function buildCreateData(values: PriceType) {

        let requestData: CreatePriceType = {
            name: values?.name || '',
            label: values?.label || '',
        };
    ...
    function buildUpdateData(values: PriceType) {

        let requestData: UpdatePriceType = {
            id: values?.id || 0,
        };
        if (values?.name) {
    ...
    async function handleSubmit(values: PriceType) {
        ...
        let response = null;
        let requestData: CreatePriceType | UpdatePriceType;
        switch (operation) {
            case 'edit':
            case 'update':
                ...
                if (!values?.id) {
                    throw new Error('PriceType ID is required');
                }
            ...
            case 'add':
            case 'create':
                if (Array.isArray(values?.priceTypes)) {
                    return;
                } else {
        ...
        <div className="row justify-content-center align-items-center">
            <div className="col-md-12 col-sm-12 col-12 align-self-center">
                ...
                {inModal &&
                    ...
                    (
                        <EditPriceTypeFields operation={operation} />
                    )
                }
                {!inModal && (
                    <Form
