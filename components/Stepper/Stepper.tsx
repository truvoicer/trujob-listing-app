...
function Stepper({ config }: StepperProps) {
  const classes = {};
  ...
  async function nextStep() {
    ...
    if (!isFirstStep) {
      if (activeStep === tabs[tabs.length - 1].id) {
        ...
        nextStep =
          tabs[tabs.findIndex((step) => step.id === activeStep) + 1].id;
      }
    }
  ...
  function renderComponent(item: StepperItem) {
    if (typeof item?.component?.node !== "function") {
      console.error("Stepper component is not a function", item);
      return null;
    }
    const nodeProps = item?.component?.props || {};
    return item.component.node({
      nextStep,
      previousStep,
      showNext,
      showPrevious,
            ...
            <div className="row">
              <div className="col-lg-12 mb-4">
                <div className="py-4 border-bottom">
                  <Nav
                    variant="pills"
                    className="d-flex nav nav-pills mb-4 text-center event-tab"
                  >
                    {Array.isArray(tabs) &&
                      tabs.map((item, index) => {
                        if (!item?.id) {
                        ...
                        if (!item?.component) {
                          console.log("Tab component is required", {
                            index,
                            item,
                          });
                          return null;
              ...
              <div className="container">
                <div className="row">
                  <div className="col-lg-12">
                    <Tab.Content>
                      {Array.isArray(tabs) &&
                        tabs.map((item: StepperItem, index: number) => {
                          if (!item?.id) {
                            console.log("Tab key is required", { index, item });
                            return null;
                          }
